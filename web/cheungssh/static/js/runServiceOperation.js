...
function goInit(){
	...
	jQuery.ajax({
		"url":initScriptForServiceOperationURL,
		"error":errorAjax,
		"beforeSend":start_load_pic,
		"complete":stop_load_pic,
		"data":{"servers":JSON.stringify(getSelectServers()["id"]),"scripts":getKey("scripts"),"parameter":JSON.stringify(window.parameterA)},
		"type":"POST",
	...
	jQuery.ajax({
		...
		"success":function(data){
			...
			if(parseInt(data.whole_progress) < 100){
				...
				window.url="command.html?serverList="+JSON.stringify(window.servers) +"&cmd=" + JSON.stringify(window.cmd)
...
function getScriptParameters(){
		var parameter = JSON.parse(getKey("parameter"))
			if (parameter.length===0){
				...
				for(var i=0;i<parameter.length;i++){
					...
					$(div).find(".parameterName").eq(0).text(parameter[i].parameterName);
					$(div).find(".parameterValue>input").eq(0).val(parameter[i].parameterValue);
					$(div).find(".parameterDescription").eq(0).text(parameter[i].parameterDescription + ".");
					tmp.appendChild(div)
				}
			}
}
