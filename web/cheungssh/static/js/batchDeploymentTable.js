...
function loadBatchDeploymentTaskList(){



    jQuery.ajax({
        "url":getBatchDeploymentTaskURL,
        "dataType":"jsonp",
        "error":errorAjax,
        ...
        "success":function(data){
            ...
            if(!data.status){
                ...
                window.alldBatchDeploymentTaskList=data.content;//记录全部的部署任务清单
                var tbody=document.getElementById("showBatchDeploymentTbody");
                for(var taskid in data.content){
                    ...
                    var serverNumber=0
		    for (id in data.content[taskid].servers){
				serverNumber+=1;
			}
                    ...
                    editButton.onclick=function(){
                        window.batchDeploymentEditTid=this.getAttribute("tid");
                        window.batchDeploymentOPTMode="edit";//记录当前编辑模
                        $("#showMainContent").load("../html/batchDeployment-create.html");
                    }
                    ...
                    startButton.onclick=function(){
                        window.currentBatchDeploymentTaskId=this.getAttribute("tid");
                    ...
                    deleteButton.onclick=function(){
                        deleteBatchDeploymentTask(this);
                    };
...
function deleteBatchDeploymentTask(deleteButton){
    //删除按钮
    var tid=deleteButton.getAttribute("tid");
    var tr=deleteButton.parentNode.parentNode;
    jQuery.ajax({
        "url":deleteBatchDeploymentTaskURL,
        "data":{"taskid":tid},
        "dataType":"jsonp",
        "error":errorAjax,
        "beforeSend":start_load_pic,
...
function startBatchDeploymentTask(){
    //正式启动部署任务
    jQuery.ajax({
        "url":startBatchDeploymentTaskURL,
        "data":{"taskid":window.currentBatchDeploymentTaskId},
        "error":errorAjax,
        "beforeSend":start_load_pic,
        "complete":stop_load_pic,
        "dataType":"jsonp",
		...
		setTimeout(loadBatchDeploymentHTML,2000)
...
$(function(){
    //绑定创建应用按钮
    document.getElementById("createBatchDeploymentApp").onclick=function(){
        window.batchDeploymentOPTMode="create";//记录当前编辑模式
        window.batchDeploymentEditTid=false;//清除此前编辑的任务id
        $("#showMainContent").load("../html/batchDeployment-create.html");
    };
    loadBatchDeploymentTaskList();//加载部署任务清单

    //绑定刷新按钮
    document.getElementById("refreshBatchDeploymentApp").onclick=function(){
       loadBatchDeploymentHTML();
    ...
    //启动部署任务的按钮
    document.getElementById("startBatchDeploymentTask").onclick=function(){
        startBatchDeploymentTask();//启动任务

    }
    //绑定部署进度查看
    document.getElementById("showBatchDeploymentDetail").onclick=function(){
        //stopShadow();
        //$("#showMainContent").load("../html/deployment_detail.html");//直接加载详情界面，但是不请求执行任务
        showErrorInfo("该功能仅适用于付费版本，如果需要，请联系作者！")
